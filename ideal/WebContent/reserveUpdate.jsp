...
<body>
	...
	<div style="min-height: 100vh; position: relative;">
		...
		<div class="titleSticky">
			<div class="titleInnerDiv">
				<h1 class="greenBack"><%=FormReplacer.fmtNull(us.getUsrName())%>様 ご予約の変更
				</h1>
			</div>
		</div>
		<center>
		...
		<center>
			...
			<div class="tableDiv bgBlur">
				...
				<div style="display: flex; justify-content: center">
					<div>
						<form id="reserveForm" action="<%=SvlUlrl.ReserveOperationSvl%>"
							method="post">
							<input type="hidden" id="mode" name="mode"
								value="<%=ReserveOperationSvl.UPDATE%>" /> <input type="hidden"
								...
								id="tableId" name="tableId" value="" /> <input type="hidden"
								id="<%=reserveVar.rsvId %>" name="<%=reserveVar.rsvId %>"
								value="<%=res.getRsvId() %>" />
							...
							<div class="formDiv">
								<div class="formDivInner">
									...
									<div>
										<select class="inputDef" id="courseId" name="courseId" onchange="coursePrint()">
								...
								%>
											<option value="<%=oneCourse.getCourseId()%>" <%=select %>><%=FormReplacer.fmtNull(oneCourse.getCourseName())%></option>
							...
							<div class="formDiv">
								<div class="flex-container">
									<div id="coursePrint" class="flex-item"></div>

									<div id="coursePrint2" class="flex-item"></div>
			...
			<input type="hidden" id="today" name="today"
				...
				id="afterDate" name="afterDate" value="<%=afterDate%>" /> <input
				type="hidden" id="rsvYyDef" value="<%=res.getRsvYy() %>" /> <input
				type="hidden" id="rsvMmDef" value="<%=res.getRsvMm() %>" /> <input
				type="hidden" id="rsvDdDef" value="<%=res.getRsvDd() %>" /> <input
				type="hidden" id="rsvHhDef" value="<%=res.getRsvHh() %>" /> <input
				type="hidden" id="rsvMiDef" value="<%=res.getRsvMi() %>" /> <input
				type="hidden" id="tableIdDef" value="<%=res.getTableId() %>" /> <input
				type="hidden" id="personDef" value="<%=res.getPerson() %>" />
		</center>
	...
	<script src="/ideal/js/reserveUpdateTableCreater.js"
		type="text/javascript"></script>
	<script>
